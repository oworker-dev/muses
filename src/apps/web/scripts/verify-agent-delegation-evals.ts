...
import { runA10FixedEvals } from "../../../packages/agent-core/tests/a10-fixed-evals"

const report = await runA10FixedEvals()
const expectedPath = expectedEvidencePath(process.argv.slice(2))

if (expectedPath) {
  const expected = JSON.parse(await readFile(expectedPath, "utf8"))
  if (JSON.stringify(expected) !== JSON.stringify(report)) {
    console.error(`A10 fixed eval evidence drifted from ${expectedPath}.`)
    process.exitCode = 1
  }
}
